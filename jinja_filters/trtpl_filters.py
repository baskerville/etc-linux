...
def lch_to_hex(lch_color):
    return convert_color(lch_color, sRGBColor).get_rgb_hex()

def hex_to_lch(hex_color):
    rgb_color = sRGBColor.new_from_rgb_hex(hex_color)
    return convert_color(rgb_color, LCHabColor)
...
def raw_rgb(hex_color):
    rgb_color = sRGBColor.new_from_rgb_hex(hex_color)
    return "{} {} {}".format(*rgb_color.get_upscaled_value_tuple())

def scaled_rgb(hex_color, component=None, factor=1.0):
    rgb_color = sRGBColor.new_from_rgb_hex(hex_color)
    values = list(map(lambda x: factor * x, rgb_color.get_value_tuple()))
